...
class DateConverter extends Controller {
    ...
    private function _get_nepali_month_in_nepali($m) {
        $n_month = FALSE;
    ...
    public function getNoOfDays($current_year, $current_month){
        foreach ($this->_bs as $year) {
            if(in_array($current_year, $year, true)){
    ...
    public function nep_to_eng_date($date){
        $date= explode('-', $date);
        $date_en= $this->nep_to_eng($date[0],$date[1],$date[2]);
        return  $date_en['year'].'-'.$date_en['month'].'-'.$date_en['date'];
    }

    public function eng_to_nep_date($date){
        $date_nep=$this->eng_to_nep($date);
        return  $date_nep['year'].'-'.$date_nep['month'].'-'.$date_nep['date'];
    }
