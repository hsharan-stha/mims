...
class SiteSearchCriteria implements CriteriaInterface
{
    /**
    ...
    /**
     * SiteSearchCriteria constructor.
     */
...
//        $this->request = $request;
    ...
    public function apply($model, RepositoryInterface $repository)
    {
        ...
        $searchFields = Request::get(config('repository.criteria.params.searchFields', 'searchFields'),
            null);
        ...
        if (!is_null($searchFields) && count($searchFields)) {
            ...
            if (count($fields) == 0) {
                throw new \Exception(trans('repository::criteria.fields_not_accepted',
                    array('field' => implode(',', $searchFields))));
            }
...
//($terms = Request::all());
//        $profile_cols = ['first_name', 'middle_name', 'last_name', 'aka_name', 'faculty'];
//        $downloads_cols = ['name', 'filename'];
//        foreach ($terms as $key => $value) {
//            if (in_array($key, $profile_cols)) {
//                $model = $model->whereHas('profile', function ($subquery) use ($key, $value) {
//                    $subquery->where($key, 'like', '%' . $value . '%');
//                });
//            }
////            if (in_array($key, $downloads_cols)) {
